...
# mySoccerNetDownloader.download(task="tracking", split=["train","valid","test","challenge"])
    ...
    def downloadGames(self, files=["1.mkv", "2.mkv", "Labels.json"], split=["train", "valid", "test"], task="spotting", verbose=True, randomized=False):
        ...
        for spl in split:
            ...
            args = list(zip(gamelist, [files]*len(gamelist), [spl]*len(gamelist), [verbose]*len(gamelist)))
            with Pool(20) as donwloader_pool:
                donwloader_pool.map(self.downloadMultiple, args)

            # for game in gamelist:
            #     self.downloadGame(game=game, files=files, spl=spl, verbose=verbose)

    def downloadMultiple(self, gameInfo):
        game, files, spl, verbose = gameInfo
        print("Starting Download of {}".format(game))
        self.downloadGame(game=game, files=files, spl=spl, verbose=verbose)
...
if __name__ == "__main__":
    ...
    mySoccerNetDownloader.password = "s0cc3rn3t"#input("Password for videos?:\n")

    # mySoccerNetDownloader.downloadGames(files=["Labels-v2.json"], split=["train","valid","test"])

    mySoccerNetDownloader.downloadGames(files=["1_224.mkv", "2_224p.mkv"], split=["train", "valid", "test"])  # download Features
